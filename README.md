...
**v1.5**:
- **Payment form now shows each employee's current due, and auto-fills the Amount.** The employee dropdown on "Record Payment" shows `(Due: ৳X)` — or `(Credit: ৳X)` / `(No due)` — right next to each name, computed from their latest settlement (`getCurrentDueByEmployee()` in `settlement.ts`, exposed via `GET /api/employees?includeDue=true`, opt-in so the plain Employees list page doesn't pay for the extra query). Selecting an employee auto-fills the Amount field with that due amount; HR can still edit it before submitting, e.g. for a partial payment.
