...
/** GET /api/export/excel?report=daily|monthly|department|employee-bill&... */
export async function GET(req: Request) {
  try {
    ...
    if (report === "daily") {
      ...
      buffer = await buildDailyExcel(mealTypeId, date);
      filename = `daily-report-${date.toISOString().slice(0, 10)}.xlsx`;
    } else if (report === "monthly") {
      if (user.role === "EMPLOYEE") throw new ApiError("Not permitted", "FORBIDDEN", 403);
      buffer = await buildMonthlyExcel(month, searchParams.get("departmentId") || undefined);
      filename = `monthly-report-${month}.xlsx`;
    } else if (report === "department") {
      if (user.role === "EMPLOYEE") throw new ApiError("Not permitted", "FORBIDDEN", 403);
      buffer = await buildDepartmentExcel(month);
      filename = `department-report-${month}.xlsx`;
      ...
      buffer = await buildEmployeeBillExcel(employeeId, month);
      filename = `bill-${employeeId}-${month}.xlsx`;
    } else {
      throw new ApiError("Unknown report type", "VALIDATION_ERROR", 422);
    }

    await writeAudit({ userId: user.id, action: "REPORT_EXPORT", entity: "Report", reason: `${report} (xlsx)` });
